...
import * as tourRepository from './tour.repository';
...
export const getTourById = async (id: string): Promise<ITour | null> => tourRepository.getById(id);
...
export const updateTour = async (id: string, tourData: Partial<ITourDTO>): Promise<ITour | null> => {
  ...
  const filteredData = Object.keys(tourData)
    ...
    .reduce(
      (obj, key) => ({ ...obj, [key]: tourData[key as keyof ITourDTO] }),
      {} as Partial<ITourDTO>
    );
